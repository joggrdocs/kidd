...
- Add a changeset (`pnpm changeset`) for every PR unless the change is dev-only (e.g. CI config, tooling scripts). Use `minor` for breaking changes (prerelease treats minor as major). Never use `major`.
